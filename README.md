...
**当前版本 v3.3** · 16 个策略组 · 11 个远程规则集 · 153 条规则(每条冗余删减均经上游列表逐条验证)
...
- **安全加固**:uTLS 指纹伪装、API 密钥、geosite/geoip 数据自动更新
...
| `proxy-override.yaml` | `rules` 开头「个人自定义直连」段 | 同上,逐条规则形式 |
...
3. 按命中情况处理:
   ...
   | 命中规则 | 症状 | 处理 |
   |----------|------|------|
   | `anti-ad` / `category-ads-all` | 国内 App 功能异常、图片加载失败 | 在自定义区加对应域名直连 |
...
脚本运行时会输出以 `[v3.3]` 开头的日志,但部分客户端不显示 `console.log`,不能作为唯一判断依据。
